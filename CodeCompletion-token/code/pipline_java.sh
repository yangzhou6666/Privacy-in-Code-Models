...
SAMPLE_RATIO=0.1 # modify SAMPLE_RATIO
...
LITFILE=../dataset/javaCorpus/literals.json
OUTPUTDIR=../save/javaCorpus
PRETRAINDIR=../save/javaCorpus/checkpoint-last       # directory of your saved model
LOGFILE=completion_javaCorpus_eval.log
# 结果在predictions_test_1.txt里
# python -u run_lm.py \
#         --data_dir=$DATADIR \
#         --lit_file=$LITFILE \
#         --langs=$LANG \
...
# 结果在predictions_train_{args.sample_ratio}.txt里，此时应该和训练阶段的数据一致
python -u run_lm.py \
        --data_dir=$DATADIR \
        --lit_file=$LITFILE \
        --langs=$LANG \
        ...
        --sample_ratio $SAMPLE_RATIO \
        --save_sample
#undo:划分训练集&测试集，基于zip(predictions_train_{args.sample_ratio}.txt,predictions_test_1.txt)
#undo:训练classifer
#验证classifier